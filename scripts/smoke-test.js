...
assert.match(publicApp, /refreshBtn'\)\.addEventListener\('click',async\(\)=>/);
assert.match(publicApp, /\/api\/mypak\/sync\/all/);
assert.match(publicApp, /mypak-login-form/);
assert.match(publicApp, /\/api\/mypak\/disconnect/);
assert.doesNotMatch(publicApp, /syncMyPakPatients\(\{silent:true\}\)/);
