...
app.get('/api/patients/:id/details', (req, res) => { const store = readStore(); const p = store.patients.find(x => x.id === req.params.id); if (!p) return res.status(404).json({ error: 'Patient not found' }); const key = normalizeName(p.fullName); res.json({ patient: computePatient(p, store.settings), medications: store.medications.filter(m => m.patientNameKey === key), medicationBalances: (store.mypakMedicationBalances || []).filter(m => String(m.patientId) === String(p.mypakPatientId)).map(normalizeMyPakMedicationBalance), prescriptions: (store.mypakPrescriptions || []).filter(m => String(m.patientId) === String(p.mypakPatientId)).map(m => ({ ...m, requestStatus: store.prescriptionWorkflow?.[String(m.prescriptionId)]?.status || 'not_requested' })), dispenseHistory: (store.mypakDispenseHistory || []).filter(m => String(m.patientId) === String(p.mypakPatientId)), doctors: store.mypakDoctors || [], scripts: store.scripts.filter(s => s.patientNameKey === key), doctorUpdates: store.doctorUpdates.filter(u => u.patientId === p.id), scriptRequests: store.scriptRequests.filter(r => r.patientId === p.id) }); });
